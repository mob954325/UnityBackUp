...
public class Test_Scene_AsyncLoad : TestBase
{
    ...
    {
        async = SceneManager.LoadSceneAsync(nextSceneName); // 비동기 로딩시작
    ...
    {
        async = SceneManager.LoadSceneAsync(nextSceneName); // 비동기 로딩시작
        async.allowSceneActivation = false;                 // 자동으로 씬 전환 막기

        while(async.progress < 0.9f) // allowSceneActivatio가 false면 progress는 0.9가 최대( 로딩완료 = 0.9 )
        {
